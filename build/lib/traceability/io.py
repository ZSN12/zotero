...
def _schema_errors(data: dict) -> list[str]:
    ...
    # P3-7（2026-09-04）：schema 定位兼容安装态。旧逻辑按仓库布局解析
    # （traceability/io.py → parents[1]/schema），pip 安装后包位于
    # site-packages，schema/ 不随包分发 → 装完即断。解析顺序：
    # ① 仓库布局（开发态）② 包内 traceability/schema（未来打包方案）
    # ③ CWD/schema（用户自带）。三者都缺时给出可操作的错误信息。
    here = Path(__file__).resolve()
    candidates = [
        here.parents[1] / "schema" / "engineering_model.json",
        here.parent / "schema" / "engineering_model.json",
        Path.cwd() / "schema" / "engineering_model.json",
    ]
    schema_path = next((p for p in candidates if p.is_file()), None)
    if schema_path is None:
        raise FileNotFoundError(
            "schema/engineering_model.json 未找到（已尝试：包上级目录、包内、"
            "当前目录）。pip 安装态请随包分发 schema/ 或在 CWD 提供。")
