#!/usr/bin/env python3
"""35A2-JC2 全册跑批：JC2 文件夹内所有图纸参与解析与工程追溯（从 JC1 版参数化派生）。
...
OVERLAY_PATH = REPO / "examples/external/guowang_35A2/layer_overlay.json"
DXF_BATCH = REPO / "out/xianyu-acceptance/batch-jc2/dxf"
OUT = REPO / "out/35A2-JC2-full-deliver"
DEMO_DIR = REPO / "web/demo/35A2-JC2"
GT_PATH = REPO / "examples/gt/35A2-JC2_ground_truth.json"
...
def sheet_bar_summary(cross_file_dir: Path) -> list[tuple[str, int]]:
    ...
    for p in sorted(cross_file_dir.glob("35A2-JC2-*.json")):
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(description="35A2-JC2 全册交付")
    ...
    if args.profile == "production_dxf":
        ...
        out_dir = REPO / "out/35A2-JC2-production"
    ...
    # D4 范围裁定：JC2 交付只吃 35A2-JC2* 图册。批次目录若混入异塔图纸
    ...
    if glb and glb.exists():
        ...
        print(f"预览: http://127.0.0.1:8000/demo/35A2-JC2/compare.html")
