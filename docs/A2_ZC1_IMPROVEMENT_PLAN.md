...
| S11e/f 73aeaa2 | 塔颈 K 撑 8 + 跳层 X 撑 4 | 66 累计 | **270** | **94.7%** |

**+54 TP（生成杆 66/66 全 TP，零 FP），R 75.8% → 94.7%（+18.9pp），
P 8.0% → 9.8%。** pure 9 不变（口径纪律）；JC1 双红线（304 /
99.6%）全程不动（含 stitch 修复后 JC1 全量重跑验证）；pytest
722 → 731（+9 单测）。
...
|---|---|---|
| 0 FN 测绘 | ✅ | 69 FN 四桶 |
| 2+4 主战场 | ✅ | 216→270（+54），六批提交 |
| 6 blind-test | ✅ 部分 | S11 通用性 fail-closed 验证完成；**盲测跑分 BLOCKED**（`run_blind_eval.py --check-split`：blind_test 切分为空，35C2-SJG1-ML.dxf 是 173 实体的材料表非结构图）。ZC1 94.7% 为 development 指标 |
...
**提分空间终判**：R 94.7% 已到诚实上限（剩余 15 FN = 同坐标
