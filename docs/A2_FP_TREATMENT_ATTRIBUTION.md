...
## 5.2 「共线断裂对 stitch 解锁」负结论（2026-09-06，三次尝试全败）

动机（离线正结果）：165 纯口径 FP 逐根归因发现 8 对共线断裂对
（两根 FP 拼接后与 GT 匹配：端点均值 <250mm、L 比 0.85-1.15、缺口
0-815mm、夹角 <6°）。离线手术式合并预示 pure TP 175→182
（P 51.5→54.6%）、dual-pure 304→312、dual-full 不变。全部 8 对卡在
`stitch_collinear_bars` 的 max_single_len_mm=800「中长杆保护」门外。

三次生产化尝试与失败机理（每次独立全管线复跑验证）：

| 尝试 | 配置 | dual-full TP | 失败机理 |
|---|---|---|---|
| 基线 | max_single=800 | 1067 | 8 对断裂对被门禁挡住（放弃 +7 TP） |
| A. 全局放宽 | 800→3000 | 1055 | kfan（panel_template_completion，role=DIAG）1267→1072；diag_synth 合成杆互拼 |
| B. 角色豁免 | exempt_roles=[DIAG] | 1055 | 模板杆本身 role=DIAG（terminal_pair_gen 396 根 + kfan 944 根）；tps@22800_24000 的 16 根 GT 匹配杆全部被拼接吞掉；另生成 8 根 6500→3000 参数化幅条合并杆 |
| C. 来源豁免 | exempt_roles=[DIAG] ∧ origins∈{dxf_geom,diag_synth,diag_complete} | 1055 | 模板杆保住了（tps@22800_24000 完好 16 根），但图纸证据域内 800-3000mm 的中长杆本来单独就是 TP（500mm 容差），互相合并成超长杆丢失匹配：219 个合并组 ≫ 目标 8 对 |

根因（结构性）：「中长杆保护」不是防误拼的保守参数，而是 1:1 匹配
语义下的必要条件——800-3000mm 的图纸证据杆单独命中 GT 的收益，
任何「允许它们互相合并」的规则都无法只针对 8 对断裂对收窄到不伤害
其它匹配（断裂对与「两根各自独立的 TP 杆」在几何上不可区分，
区分度只存在于 GT 对照之后，而那是评测不能回灌生产的）。

决策：**三次尝试全部回退，overlay 豁免键删除，恢复 max_single=800
基线**。代码保留 `max_single_exempt_roles/origins` 参数（默认 None =
行为与历史完全一致的 no-op）作为已否决方案的接口存档。

验证（回退终态，两次复跑确定性确认）：full TP=913 FP=2451（P=27.1%
R=85.2%）、dual-full TP=1067（R=99.6%）、dual-pure TP=304（P=63.5%）、
A1 168/197（P=100% R=85.3%）、L6 PASS、web 镜像 sha 一致、
pytest 736/5。离线 +7 TP 判定为不可生产化，记录于此。
...
- 共线断裂对 stitch 解锁（全局放宽/角色豁免/来源豁免）：**不做**
  （§5.2 三次尝试全败，中长杆保护是 1:1 匹配语义的必要条件）。
