...
## 11. 待决问题清单与下一步建议（2026-08-31，Round 10 收尾）

> 本清单供评估后接手。已完成的修复都已提交；以下是仍未闭环的问题，
> 按「是否阻塞 A2 85%」和「是否有解」分四类。**核心结论：A2 P/R≥85% 被
> 图纸↔GT 固有差异阻塞，代码侧已无在不违反约束前提下的达标准路径。**

### A. 阻塞 A2 85%、且无解（需决策是否接受/降标）

| # | 问题 | 证据 | 结论 |
|---|------|------|------|
| A1 | 底段 z[0,5500] 立面图纸缺失 | 40=加工详图、00-2/01-2=标题页、02 只画顶部横担段 | 无法从图纸还原 GT ±2762 底段 |
| A2 | GT 多变体叠加 | GT 06 每塔角 8 段重叠主材（L100X7/X8，z 跨互相重叠） | 物理不可复现，召回天花板 <100% |
| A3 | 图纸节间稀疏 | 图纸 06 仅 2-3 个 X 面板边界，GT 有 10 个节间水平 | 无法细分到 GT 全部水平 |
| A4 | 长杆缺口 | 模型 6 根 vs GT 56 根 >3000mm；图纸斜材「短画+通长」，GT「中长段」 | 拓扑不一致，无法补足 50 根 |

**这四项的共性**：都不是代码 bug，而是「提供的 DXF 图纸」与「GT 参考」之间的
固有拓扑差异。跨越它们的唯一途径是「按 GT 拓扑重建斜材」或「注入 GT 坐标」，
均被计划约束明确禁止。**建议决策**：A2 85% 目标改为「A2 相对提升」或接受
当前天花板（约 2-20%），或放开「GT 拓扑」约束。

### B. 阻塞 A2、但有边缘修复空间（低收益）

| # | 问题 | 现状 | 建议 |
|---|------|------|------|
| B1 | 02 横担图纸不完整 | 图纸主横担 x∈[34406,34490]=±840mm，GT ±2200mm | 图纸本身没画全横担，扩 region 无法补足；低优先级 |
| B2 | 40 从 merge 移除 | 40 在 merge_stems_extra，但它是加工详图 | 移除会让底段完全缺失（A1 已说明底段无图）；暂不建议移除，除非接受底段真空 |

### C. 已发现但未修复的代码缺陷（可继续修，不阻塞 A2 达标）

| # | 缺陷 | 位置 | 风险 |
|---|------|------|------|
| C1 | 共线合并跨 region 泄漏 | `_merge_collinear_fragments`（tower_dxf.py L292-398），gap_tol=30u 可跨 region 边界「走」60u 进入材料表线簇（N154 即此因） | 建议限制链延伸不越出种子段 region，或合并后按 region 过滤节点 |
| C2 | `_subdivide_at_levels` 实验性未启用 | tower_dxf.py，需 overlay 显式 `subdivide_at_levels` 才触发；实测 06 只能命中 GT 的 11500/12000 两级（Δ7-21mm），其余 Δ200-300mm | 已提交但收益有限，是否启用由后续评估决定 |

### D. 三个子代理的最终状态（已全部回收）

| 子代理 | 任务 | 结论 | 是否已采纳 |
|--------|------|------|-----------|
| fd3da879 | 段02材料表污染 | 主修复 9a0774b 已验证（34701→2168.75mm）；补发现残留缺陷 A（外推爆炸） | ✅ 残留缺陷 A 已修（68303ad）；C1 未修 |
| b0fd2738 | 段40/04缺几何 | 40=加工详图（推翻 f2f6997）；04 实为正确（±1082 非 ±2200）；底段立面缺失 | ✅ 40 已回滚；04 认知已修正；底段缺失已记录 |
| 5399c735 | 节间细分方案 | 方案 A（斜材端点聚类+只切主材）；GT 多变体叠加（8 重叠腿）；比例 20mm/u 三重验证 | ✅ 方案 A 已实现（6819bcb）；多变体已记录 |

### E. 本轮（Round 9-10）提交记录

| commit | 内容 |
|--------|------|
| 68303ad | 回滚段 40 错误 region + 外推钳位（残留缺陷 A） |
| 6819bcb | 节间细分方案 A（`_subdivide_at_levels`，实验性） |
| 035d7af | 文档 10.9：段 40 加工详图、底段立面缺失、04 认知修正 |

### F. 当前基线（Round 10 全量回归，纯 ezdxf，全新输出目录）

```
A2 几何检测:  TP@500=20  P=3.9%  R=1.9%   (基线 16/2.2%/1.5%)
A3 件号关联:  0.0% (0/20)
A1 件号识别:  P=80.3%  R=47.7% (94/197)
门禁:         失败 (140 Degree=1 悬空节点; 276 横担悬臂端头属正常)
塔高 bbox Z:  [0.0, 36606.45]
```

### G. 建议的后续路径（供评估后选择）

1. **若接受 A2 降标**：继续修 C1（共线合并跨 region 泄漏）+ 门禁悬空节点，把
   非 A2 指标（A1 件号、门禁、3D 拼装）打磨到可用，A2 按「相对提升」交付。
2. **若坚持 A2 85%**：需放开「GT 拓扑」约束（允许按 GT 面板拓扑重建斜材），
   这是唯一能跨越 A1-A4 的路径；否则目标不可达，应标记 blocked。
3. **若转向数据**：确认是否缺了底段立面图纸（向数据方索取 z[0,5500] 的塔身
   立面图），若能补图，A1 可解，其余 A2-A4 仍需 GT 拓扑决策。
