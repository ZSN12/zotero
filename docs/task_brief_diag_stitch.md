...
**根因已定位（子代理取证，直接采信，不必重新诊断）**：
`stitch_collinear`（centerline_extract.py:94-152）的追加型 join（L129-134）
**无方向检查**：t 投影未钳位（L138-141），端点相邻(≤6u)且垂距≤1.5u 的段
即替换链端点，即使新段另一端落在链跨度**内部** → 链收缩。同一斜杆在
图层 0/1/4 画 2-3 遍（相差 1-3u）时链来回塌缩：
- 05 册：3901mm→**30mm**（10 段成员）、1953mm→74mm
- 07 册：5463mm→30mm；04 册：2999mm→25mm
- 线级污染面：04: 26/54、05: 48/84、06: 28/68、07: 24/56

塌缩后的链 <min_cand_mm=300（L690）被丢弃——这就是 285→16 的主因。

**修法方向**：join 加方向检查——free 端（新段的远端）在链方向上的投影
必须在被替换端点的投影**之外**（t_free > t_end，允许小容差），否则拒绝
join。改完用第 0 节快速闭环量 05 册 ≥15u 斜线条数。

其它已排除的环节（不要浪费时间）：pair_double_lines 损失仅 1-2 线/册；
tower_dxf 后处理（merge/stitch/subdivide）对斜杆 centerline **零损失**
（全重放验证 12/16/16/16 centers 全部存活）；`tower_symmetry.py:247-283`
min_diag_len_mm=500 另损失 05:4、06:18、07:4、04:2（可顺带评估是否
收紧到 400，但需全管线验证 dual 不破）。
