...
**2026-09-02 实测修订**（详见 `docs/PHASE1_06_DIAGNOSIS.md` §五）：
候选提取器已落地（`scripts/eval_segment_candidates.py`，覆盖率口径）：
06 段 **84.5%**、07 段 **82.7%**（水平 32/32 全中、退化全中、腿 48/52 与 40/44）。
剩余 ~16% 全部是「中心↔角」K 撑/扭转杆——图纸 X 撑节点层位
（13000/15000/17000）与 GT 斜杆端点层位（14000/14400/14500/16000）结构性
不同，端点距离 ~990mm 超容差，宽角度口径无增量。**单册前视直读的
候选召回上限 ≈ 85%/83%**；`≥95%` 验收线需修订为按杆类分层
（腿/水平 ≥95% 已达成；斜杆改双视图/参数化通道另立验收，
06/07 册无侧视图，需 A2-parametric 或结构规则通道）。
