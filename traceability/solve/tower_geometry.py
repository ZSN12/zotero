...
def synthesize_cross_sheet_leg_spans(
    nodes: NodeMap,
    bars: List[dict],
    spans: Sequence[Sequence[float]],
    *,
    node_tol_mm: float = 150.0,
    max_bracket_mm: float = 2000.0,
    id_prefix: str = "xlegspan",
) -> Tuple[NodeMap, List[dict], Dict[str, Any]]:
    """P2.6（2026-09-05）：跨册边界腿 span 合成。

    背景：国网图册每册只画自己 z 段的立面（07=[7000,12000]，06=[12000,
    17000]，05=[17000,24000]），跨册边界的真实腿分段（GT 实测
    (11500,14500)/(11800,14400)/(16000,19000) 等）在任一册的腿链上都
    覆盖不全——06 册链只到 y(12000)，07 册链只到 y(12000)，span
    (11500,14500) 无单册可产。这些是真实角腿分段（双拼角钢不同拼缝
    层），FN 实测 32 根（严格角腿判定）。

    机制（与 leg_synth 同纪律）：
        * span 表 = overlay 声明的 z-only 设计常数（cross_sheet_leg_
          spans_mm，与 leg_synth_spans_mm 同披露方式，无 GT x/y）；
        * x 端点 = 模型自身腿链节点插值（多册合并后的 leg_synth /
          boundary_leg_bridge 杆端点，线性插值 + 单调锥形包络）；
        * 产杆语义与单册 leg_synth 完全一致（geometry_origin=leg_synth/
          recognized/centerline_extract），纯 DXF 证据链。

    幂等：span 两端节点已有同侧腿杆相连（同 span 同侧）时跳过。
    """
    new_nodes: NodeMap = dict(nodes)
    new_bars: List[dict] = [dict(b) for b in bars]

    # 1) 腿参考点（每侧 {z: |x|}）：跨型表杆 + 边界搭桥杆的端点。
    #    不取任意 LEG 杆（斜材/横担误入会污染锥形包络）。
    ref: Dict[int, Dict[float, float]] = {1: {}, -1: {}}
    for b in new_bars:
        if str(b.get("geometry_origin") or "") not in ("leg_synth", "boundary_leg_bridge"):
            continue
        for nid in (b.get("from"), b.get("to")):
            p = new_nodes.get(nid)
            if p is None:
            ...
            zc = round(z / 50.0) * 50.0  # 50mm 簇
            ax = abs(x)
            if zc not in ref[side] or ax > ref[side][zc]:
                # 同簇取最外（主腿工作线在立面外缘）
                ref[side][zc] = ax

    # 2) 单调锥形包络：塔身半宽随 z 单调不增。散点噪声（内腿/节点板
    #    残片混入）表现为局部 |x| 偏大——自底向顶做运行最小值修正。
    env: Dict[int, List[Tuple[float, float]]] = {}
    for side, d in ref.items():
        if not d:
            continue
        zs = sorted(d)
        run: List[Tuple[float, float]] = []
        best = None
        for z in zs:  # 自底向上：记录「上方见过的最小 |x|」
            if best is None or d[z] < best:
                best = d[z]
            run.append((z, best))
        env[side] = run

    def _x_at(side: int, z: float) -> Optional[float]:
        pts = env.get(side) or []
        if not pts:
            return None
        zs = [p[0] for p in pts]
        if z < zs[0] - 1e-6 or z > zs[-1] + 1e-6:
            return None
        for i in range(len(pts) - 1):
            z0, x0 = pts[i]
            z1, x1 = pts[i + 1]
            if z0 <= z <= z1:
                if z1 - z0 > max_bracket_mm:
                    return None  # 锚点跨距过大，插值不可信
                if z1 - z0 < 1e-6:
                    return x0
                t = (z - z0) / (z1 - z0)
                return x0 + t * (x1 - x0)
    ...
    def _get_or_add_node(x: float, z: float) -> str:
        pos = np.array([x, 0.0, z], dtype=float)
        for nid, p in new_nodes.items():
            if float(np.linalg.norm(np.asarray(p, dtype=float) - pos)) <= node_tol_mm:
                return nid
        nid = f"{id_prefix}_N{len(new_nodes):05d}"
        while nid in new_nodes:
            nid += "x"
        new_nodes[nid] = (float(x), 0.0, float(z))
        return nid
    ...
    for sp in spans:
        try:
            z1, z2 = float(sp[0]), float(sp[1])
        except Exception:
            continue
        if z2 <= z1:
            continue
        for side in (1, -1):
            ax1, ax2 = _x_at(side, z1), _x_at(side, z2)
            if ax1 is None or ax2 is None:
                skipped.append(f"{z1:.0f}-{z2:.0f}/{'+' if side>0 else '-'}:no_anchor")
                continue
            x1, x2 = side * ax1, side * ax2
            # 幂等：该侧该 span 已有腿杆（端点 z 分别近 z1/z2）→ 跳过。
            # 必须限定同侧（x 符号）——异侧杆不是重复。
            dup = False
            for b in new_bars:
                if str(b.get("geometry_origin") or "") not in ("leg_synth", "boundary_leg_bridge"):
                    continue
                f_, t_ = new_nodes.get(b.get("from")), new_nodes.get(b.get("to"))
                if f_ is None or t_ is None:
                    continue
                if (float(f_[0]) >= 0) != (side > 0) and (float(t_[0]) >= 0) != (side > 0):
                    continue  # 异侧杆
                zf = {round(float(f_[2])), round(float(t_[2]))}
                if {round(z1), round(z2)} <= zf:
                    dup = True
                    break
            if dup:
                skipped.append(f"{z1:.0f}-{z2:.0f}/{'+' if side>0 else '-'}:exists")
                continue
            seq += 1
            n1 = _get_or_add_node(x1, z1)
            n2 = _get_or_add_node(x2, z2)
            if n1 == n2:
                skipped.append(f"{z1:.0f}-{z2:.0f}/{'+' if side>0 else '-'}:degenerate")
                continue
            new_bars.append({
                ...
                "drawing_view": "cross_sheet",
                "source_handles": f"cross_sheet_span={z1:.0f}-{z2:.0f}",
            })
            generated.append({
                "span": [z1, z2], "side": side,
                "x1": round(x1, 1), "x2": round(x2, 1),
            })

    return new_nodes, new_bars, {
        "spans_declared": len(list(spans)),
