...
验收：
  * extrapolate_base_segment 生成杆 100% 带 parametric_struct 分类
    （parametric_leg / parametric_spoke——S8 裙部 fan-spokes 拓扑）；
  * 分类与 role 一致（LEG → parametric_leg，DIAG → parametric_spoke）；
...
class TestParametricStruct(unittest.TestCase):
    ...
    def test_all_bars_classified(self):
        ...
        for b in bars:
            self.assertIn(
                ...
                ("parametric_leg", "parametric_spoke"),
    ...
    def test_class_matches_role(self):
        ...
        for b in bars:
            expect = "parametric_leg" if b["role"] == "LEG" else (
                "parametric_spoke" if b["role"] == "DIAG" else None)
    ...
    def test_counts(self):
        ...
        spokes = sum(1 for b in bars if b["parametric_struct"] == "parametric_spoke")
        self.assertGreater(legs, 0)
        self.assertGreater(spokes, 0)

    def test_no_spokes(self):
        _, bars, _ = self._run(add_spokes=False)
        for b in bars:
