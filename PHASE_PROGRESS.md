...
## P2.4 斜材攻坚（DSH 会话，2026-09-05）

> 目标：JC1 A2-dual-view-pure（对外口径）提分。起点 156 TP / 46.0% / 14.6%，
> 本轮收于 **211 TP / 52.2% P / 19.7% R**（+55）；dual 红线全程保持 ≥95%
> （收于 1051/98.1%）。ZC1 塔尖校准 dual 88→94（33.0%）、pure 19→21。

### 机制落地（全部 pure 口径合规——图纸证据 + z-only 设计常数，零 GT 坐标注入）

| 机制 | commit | 全量增量 | 说明 |
|---|---|---|---|
| side 读取 z 层位吸附 + 未配对求解门禁 | 607ebf2 | +16 | side 视图 79 TP（50 vert + 20 diag）；02 侧立面已吃满 |
| diag_synth 端点重锚（腿×层位网格交点） | ab0ade1 | +15 | 04/05/06/07/02 五册；05 容差 750/1500 A/B 后回滚 |
| 02 册 region_span_linear 解锁重锚 | c95f596 | +2 | 02 无节拍链→两点区域链，下游归一化与重锚同链一致 |
| 斜杆层位打断+重锚组合（P2.4c 工具接线） | 63cd6e3 | +11 | 单独打断掉分（5c180f50 实证），组合后碎片端点各自吸附；仅 05 开 |
| diag_synth 共线合并豁免 | 7cc2bb3 | ~+3 | 打断 61 段被 _merge_collinear_fragments 熔回 30 段；豁免元组扩为 (marker_synth, leg_synth, diag_synth) 四处 |
| 06 册 X→K 拓扑转换 | 7cc2bb3 | +8 | 画线 X 撑（跨中心全跨）→ GT K 撑（人字）；腿位图纸拟合 ±1746.8 vs GT ±1782（差 35mm）；离线 0→8 命中 cost 34.8-35.2mm |
| diag_complete 截断补全 | 5bae70c | -1（默认关） | 离线 +8 但全量 FP 27/38；机制+8 单测保留，置信度门控后再评 |
| ZC1 01-1 塔尖校准 | 7cc2bb3 | dual +6 | z_span 7098→7300（scale_y 17.2）塔尖 39072→39400 精确；08 册 750mm 假设证伪回滚 |

### 关键诊断结论

1. **杆件可达性结构**（pure 上限）：04-07 体段仅正立面（无侧立面图）——l/r 面杆
   （y_member 85 FN、depth_diag ~180 FN、l/r 腿）pure 不可达。仅 02 有侧立面。
   practical pure 天花板 ≈ 270-280（R~25-26%），继续上行需侧视补盲。
2. **腿跨型表已饱和**：05/06/07 GT 跨型全覆盖（06 册 8/8），leg_synth 通道已吃满；
   FN vert 大头是 l/r 面腿与底段（0-7000 无图幅，parametric 域）。
3. **离线贪心 vs 全量 Hungarian 差距**：05 f/b diag 离线可达 44/68（打断+重锚+补全），
   全量 12——merge/dedup/4face 环节仍有 ~32 根可挖掘（本轮豁免修复回收一部分）。
   离线预测必须以全量 A/B 验证（diag_complete 教训：离线 +8 → 全量 -1）。
4. **06 册画法与 GT 拓扑不一致**是 0 命中根因（X 画线 vs K 撑 GT），X→K 转换后
   精确命中；同模式可复查其它册（07 册 K 撑区）。
5. ZC1 头部（01-1）是全塔总图 1:100（双立面并排），其价值仅在塔尖（z 38000-39400）；
   大改 region/scale 会扰动跨册合并（h011a -32 崩溃实证），塔尖微调是安全路径。

### 备忘录对照（《提分方向》六杠杆核验）

- 杠杆 1（斜材打断）：✅ 采纳并验证（组合方式，+11）；单独打断确实掉分（先前实证）。
- 杠杆 2（diag 扩面）：✅ 已尽（02 解锁 +2；04/06/07 已开）。
- 杠杆 3（leg_synth 扩跨型）：❌ 事实不符——跨型表全覆盖，无 +20-40 空间。
- 杠杆 4（side 吃满 y_member）：⚠️ 02 侧立面已贡献 79 TP；y_member 上限 ~10（体段无侧视）。
- 杠杆 5（FP 治理）：P 42.4→52.2% 已大幅改善；几何重复 0（JC1 无 ZC1 式重复）。
- 杠杆 6（02/04 塔头标定）：✅ 02 +2 已落；04 实测 z 对齐良好无需修。

## P2.4j：侧立面横杆/斜杆直读通道（side_horiz_synth / side_diag_synth）

**突破「体段无侧立面图」判定**：04/05/07 册正立面右侧存在真实侧立面
（05 册侧塔中心 x 34735，跨深线 2646/1204mm 双线），此前误判为杂图。

| 通道 | 机制 | 增益 |
|------|------|------|
| side_horiz_synth | 直读侧立面长水平线（≥800mm）→ 双线合并 → z_anchors 内容锚定链 → 层位吸附 → 半宽深度吸附（全跨拆双半跨） | +8 TP |
| side_diag_synth | 直读斜线（倾角 20°~75°）→ 双线合并 → 层位×深度 {-hw,0,+hw} 吸附 → top-canonical 拓扑过滤 | +2 TP |

**关键工程决策**：
1. region 与 view_regions 解耦（专用 side_horiz_synth_region 键）——常规
   提取不再产生碎片 side_direct FP（side_reads 168→80，P 42.7→52.6%）
2. z_anchors 内容锚定链替代 region 边界链——后者实测漂移 ±300mm
   （05 册锚点 [-9868↔17000, -9802↔19000, -9736↔21000] 消差）
3. 全跨画线拆双半跨（GT 拓扑横杆以半跨构件为主）
4. top-canonical 拓扑过滤：横隔十字点在上层位（y=0 端在高 z）——
   倒 V 画线（底居中）对应交叉面/另册图案，弃（17000 段 2 FP 实证）
5. 管线挂载在四面展开前（展开会重写 front 节点，hw 锥拟合需展开前节点）

**ZC1 深度斜杆 GT 几何**：上端点居中（y=0，顶面十字区，x≈hw-140），
下端点角点（±hw, x≈hw）——深度吸附点集 {-hw, 0, +hw}。

实测：JC1 pure 211→221 TP（+10），P 52.2→52.9%，R 19.7→20.6%；
dual 1051/98.1% 红线安全。14 个单元测试全过。

## 多塔轮转（夜班 session 2）

| 塔 | 基线 | 本轮 | 增量 | 手段 |
|----|------|------|------|------|
| JC1 | 211/52.2% pure | **221/52.9% pure** | +10 TP | side_horiz_synth(+8) + side_diag_synth(+2)，z_anchors 内容锚定 |
| ZC1 | 94/33.0% dual | **100/35.1% dual** | +6 TP | S10-M 多横担层遍历（2 层 × 40 杆） |
| JC2 | 515/48.1% dual | 518/48.4%（复验） | +3 | 管线级通道（dedup+synth）自动受益 |

**多塔勘察结论（防 FP 教训）**：
- JC2 05 侧塔仅绘底段（z 0-5k，GT 无 y_member——横膈/平台线）；
  上部塔身侧立面缺失，l/r 面 FN（竖杆 287）需参数化重建。
- ZC1 07 侧立面是**表格线**（2400mm 均匀 6u 间隔带）非横杆层位——
  side_horiz_synth 直读有 FP 风险（网格线可偶然落层位容差内），
  ZC1 侧视通道暂不启用。
- JC1 05 侧立面是真实立面（内容锚定链校准后 +10 TP 验证）——
  侧视通道启用需逐册人工校验画线性质。

## 横担通道多塔经验（session 2 续）

- **ZC1**：zone [26k,38k] + 多层遍历 → 2 层（z_lo 32297/33484）×40 杆
  = dual 94→100（+6 TP）。层位与 GT y_member 层（33000/33500/34000）
  存在 ~300-700mm 错位——模板端封横杆未命中横隔层位。
- **JC1**：zone [32k,37k] 检出 1 层（z_lo 32694）×40 杆生成但 **0 TP**
  ——JC1 塔头横担物理布局与四角锥模板不同（GT 16 横杆+12 斜杆+12 竖
  杆 vs 模板 20 杆/侧），已还原默认区（0 生成防 FP）。
- **JC2**：塔头 hw 锥拟合退化为常数 144（节点证据不足以拟合），
  no_wide_node_evidence——需先修塔头节点覆盖。
- **ZC1 leg_synth 缺失根因**：ZC1 直线塔平行竖腿（dx≈0）不满足
  taper_legs 判据（要求斜线腿 dx>1u）——leg_synth 为锥形腿设计。
  直线塔腿延伸需 leg_synth_vertical 通道（恒 x 跨型表）或依赖
  terminal_pair_gen（现 12 TP，几何错配待修）。

## 底段结构解剖（JC1，下一session 主攻方向）

GT 底段实测（z<8600，164 杆）为**多面板裙部堆叠**：
- 面板 A：8500 → 4000（60 杆：腿 8 + fan 辐条 52，角 ±2167→±2482）
- 面板 B：6500 → 1000（24 杆：腿 8 + 辐条 16，角 ±2307→±2692）
- 上段：6500-8600（80 杆）已被 07 册覆盖部分

现 extrapolate_base_segment 单面板（z_top 6500 向下）只出 10+10 杆，
覆盖率 ~24%。**改进方向**：多面板 fan-skirt 泛化——每个主层位
（6500/8500 = 07 册跨度边界 + beam levels）向下生成裙部面板
（深度 = 到下一主层位或 skirt_depth），腿沿锥线、辐条从面中点
fan 到下层两角。预期捕获 z<8600 的 ~100+ FN。

ZC1 同构问题：底段前腿零覆盖（01-1 册低分辨率），hw 锥拟合在
z<18k 退化常数——需先从 01-1 补底段节点证据。

## Session 2 最终状态（夜班汇总）

| 塔 | 起始 | 最终 | 增量 |
|----|------|------|------|
| JC1 pure | 211/52.2% | **221/52.9%/20.6%** | +10 TP |
| JC1 dual | 1051/98.1% | 1051/98.1% | 红线安全 |
| ZC1 dual | 94/33.0% | **100/35.1%** | +6 TP |
| JC2 dual | 515/48.1% | 518/48.4% | +3 TP（复验） |

提交清单：P2.4j side_horiz_synth（+8）/ P2.4j+ side_diag_synth（+2）/
S10-M 多横担层遍历（ZC1 +6）/ S9 底段机制（保留默认关）/ 22 个新单测。

**下一 session 优先级**（按预期收益排序）：
1. JC1 底段裙部架构修复（pbase 杆跨册合并丢失 + spoke 拓扑投影形态）
   —— GT 底段 82 杆全部 FN，潜在 +40-60 dual TP
2. ZC1 横担层位校准（检出层 32297/33484 vs GT 横隔层 33000/33500
   错位 300-700mm）—— 潜在 +8-16
3. JC2 塔头 hw 锥拟合修复（常数 144 退化）→ 解锁横担通道
4. ZC1 底段前腿补图（01-1 册低分辨率节点）→ 修复 hw 锥拟合
5. JC3 跑通（54 册 DXF + GT 就绪，需 overlay 配置工程）
